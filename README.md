...
> 출시 기간 : 2024.04.10 ~ 2024.05.05   
 ...
 Crewous에서는 농구 크루를 직접 생성, 검색 또는 추천 받을 수 있습니다. 크루에는 크루 가입 비용이 존재하며, 크루에 가입 시 결제가 진행됩니다. 생성된 크루의 정보창에 들어가게 된다면, 크루 정보 및 멤버를 확인할 수 있으며 크루별로 댓글을 달 수 있습니다.
...
- `포트원`을 통해 결제 기능 연동 경험 및 영수증을 통한 유저의 결제 유효성 검증단계 로직을 실제로 구현해보는 경험
...
탭을 담당하는 CollectionView의 셀을 클릭했을 때 멤버 탭의 화면 전환 이 후 아무런 데이터도 노출되지 않은 문제가 있었습니다.

#### ❗️ 원인파악

Page ViewModel에서 `PostData`와 `UserData`를 넘겨주고 PageVC에서 다시 `InfoVC`와 `MemberVC`에 각각 PostData와 UserData를 넘겨줍니다.

다만 PageVC는 첫 번째인 `InfoVC만 ViewDidLoad`가 되기 때문에, 
두번째 VC인 MemberVC는 UserData를 넘겨줘도 아직 `시퀀스 구독 이전이기 때문에` 데이터 전달이 이루어지지 않았습니다.

<img src = "https://github.com/DONOTINTO/Crewous/assets/123792519/ec29c25c-2988-42c1-89e7-9aaebfba4bf5">

#### ❗️ 해결방법

셀을 클릭해서 실제 MemberVC가 ViewDidLoad 된 후 값을 넘겨줬습니다.
...
4. seleted row를 통해 선택된 page를 확인하고 해당하는 viewModel에 다시 한번 데이터를 전달, 이 후 seleted row 데이터가 전달받을때마다 동작을 위해 zip대신 combinelastest를 사용
...
화면전환 시 ViewModel로 값 전달 방식과 타이밍의 통일성이 없어 가독성을 떨어트리고 유지보수를 어렵게 만들었습니다.
특히 아래와 같은 방식은 다른 VC에서 ViewModel의 프로퍼티에까지 접근할 수 있기 때문에 객체지향 프로그래밍 관점에서 불필요한 접근으로 보여졌습니다.

또한 값 전달이 VC 생성 이후에 이루어지기 때문에 아래와 같이 completion에 의존했습니다.
...
의존성 주입 개념을 학습하고 화면전환 시 ViewModel을 생성하여 VC 초기화 때 주입하였습니다.
